...
def cov_parallel(spk_train, neuron_pairs=None, dp=1,n_processes = 16):
    ...
    if neuron_pairs is None:
        neuron_pairs = [[i, j] for j in range(n_neuron) for i in range(j+1)]
    ...
    for batch in range(n_batch):
        ...
        with ProcessPoolExecutor() as executor:
            test = [executor.submit(cov_estimate, spk_train=spk_train[:int(n_time_step*dp),:], N_i=i, N_j=j, filename=f"cov_{i}_{j}_{int(n_time_step*dp)}" ) for i,j in neuron_pairs_batch]
    # print(total)
if __name__ == "__main__":
    N, Nt = 64, 1000000
    spk_train = load_spk_train(N=N, Nt=Nt, filename=f"spk_train_{N}_{Nt}_b-2_weight_2")

    cov_parallel(spk_train.spike_train)
