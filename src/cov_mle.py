...
def cov_parallel(spk_train, dir_name, neuron_pairs=None, dp=1,n_processes = 16):
    ...
    for batch in tqdm(range(n_batch)):
        ...
        with ProcessPoolExecutor() as executor:
            test = [executor.submit(cov_estimate, spk_train=spk_train[:int(n_time_step*dp),:], N_i=i, N_j=j, dir_name=dir_name, filename=f"cov_{i}_{j}_{int(n_time_step*dp)}" ) for i,j in neuron_pairs_batch]
    # print(total)
if __name__ == "__main__":
    ...
    for dp in [1]:
        ...
        cov_parallel(spk_train.spike_train, dp=dp, n_processes=16, dir_name="Spk64_2m_Data_volume_obs_-1_diag_covariance_EI_network_weight_4")
